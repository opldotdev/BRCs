# BRC-507: 1Sat Ordinals — Inscription Envelopes
...
This BRC defines how **inscriptions** attach content to a [BRC-506](./0506.md) token: an `ord` **envelope** embedded in the locking script of a **1-satoshi** output.
...
See [BRC-506](./0506.md) for origin, sat ordering, and transfer.
...
* Chain identity (origin and transfers) is unchanged by envelopes — see [BRC-506](./0506.md).
...
Because envelopes live in **outputs** (not input witnesses), an inscription is identified by the outpoint of the output that carries it, written per [BRC-505](../outpoints/0505.md).
...
Transfers afterward follow [BRC-506](./0506.md) (1-satoshi input → 1-satoshi output under sat ordering). The envelope does not need to be repeated on every output in the chain unless the application re-inscribes.

### Metadata (MAP)

Optional application metadata MAY follow the inscription on the same output after `OP_RETURN`, using the **Magic Attribute Protocol** ([BRC-503](../scripts/0503.md), protocol id `1PuQa7K62MiKCtssSLKy1kh56WWU7MtUR5`):
...
* Full MAP opcode encoding — see [BRC-503](../scripts/0503.md) and [BRC-500](../scripts/0500.md)
* Collection schemas and in-envelope MAP field layout — specified separately (1Sat Collections)
* Issuer authenticity — see Sigma ([BRC-504](../scripts/0504.md))
...
Those may require [BRC-506](./0506.md), this envelope BRC, or both.
...
1. [BRC-506: 1Sat Ordinals — Single-Satoshi Tokens and Origin Tracking](./0506.md)
2. [BRC-505: Outpoint Content Addressing](../outpoints/0505.md)
3. [BRC-503: MAP — Magic Attribute Protocol](../scripts/0503.md)
