...
Any other MAP keys on the same command are **join tags**. A client that has one identifier can look up the others on that output. Key spellings for global identifiers (`url`, `tx`, `btctx`, `ethtx`, `topic`, `upc`, `name`, and others) are MAP's shared keyspace, defined in BRC-503 §5; this BRC does not own them and does not restate the list.
