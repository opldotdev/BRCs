# BRC-512: Bitcoin Schema — Social Data Types
...
Every Bitcoin Schema social record is a data-carrier output composed with the [BRC-500](../scripts/0500.md) pipeline:

```
[B <content> <mediaType> <encoding>] | MAP SET app <appname> type <type> [<key> <value> ...] | AIP <algorithm> <address> <signature>
```

- **Content** (when the type carries content) is a B segment ([BRC-501](../scripts/0501.md)).
- **Attributes** are a MAP `SET` segment ([BRC-503](../scripts/0503.md)) carrying at minimum `app` (the producing application's name) and `type` (the schema type).
- **Authorship** is an AIP signature ([BRC-502](../scripts/0502.md)) covering the fields to its left. Records SHOULD be AIP-signed; unsigned records have no verifiable author.
- **Identity** references use BAP identity keys ([BRC-511](../peer-to-peer/0511.md)) under the key `bapID`.
...
Tags categorize content for search and filtering, carried as a MAP `ADD` (list semantics, [BRC-503](../scripts/0503.md)) in an additional output of the same transaction:
...
Attachments carry rich media alongside a post or message, each as a separate B output ([BRC-501](../scripts/0501.md)) in the same transaction, AIP-signed like any other output:
...
- **Unsigned records** — anything can be written under any `app` name and `type`; only the AIP signature binds a record to an author, and only a BAP identity chain ([BRC-511](../peer-to-peer/0511.md)) binds that author to a persistent identity. Indexers SHOULD treat unsigned records as anonymous and unverifiable.
...
- [BRC-500: Bitcom](../scripts/0500.md) — pipeline composition
- [BRC-501: B — Bitcoin Data Protocol](../scripts/0501.md) — content and attachments
- [BRC-502: AIP — Author Identity Protocol](../scripts/0502.md) — authorship signatures
- [BRC-503: MAP — Magic Attribute Protocol](../scripts/0503.md) — attribute layer (`SET`, `ADD`)
- [BRC-511: BAP — Bitcoin Attestation Protocol](../peer-to-peer/0511.md) — identity keys referenced by `bapID`
